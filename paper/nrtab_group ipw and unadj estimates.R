...
nfhs4_group_estimates <- bind_rows(
  ...
  dplyr::mutate(ipw_interval = paste0(proportion_ipw," (",proportion_low_ipw,", ",proportion_upp_ipw,")"),
                unadj_interval = paste0(proportion_unadj," (",proportion_low_unadj,", ",proportion_upp_unadj,")")) %>% 
  dplyr::select(strata,population,id,indicator,contains("interval")) %>% 
  ...
  pivot_wider(names_from=c(population,indicator),values_from=c(ipw_interval,unadj_interval))
