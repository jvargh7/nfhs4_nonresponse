...
nfhs4_ipw_estimates <- bind_rows(child_indicators_state %>% mutate(population = "Child"),
                                 male_indicators_state %>% mutate(population = "Men"),
                                 female_indicators_state %>% mutate(population = "Women")) %>% 
...
nfhs4_ipw_estimates_district <- bind_rows(child_indicators_district %>% mutate(population = "Child") %>% rename(sdistri = shdistri),
                                          female_indicators_district %>% mutate(population = "Women")) %>% 
  dplyr::mutate(bias_ipw_orig = proportion_ipw - proportion_orig,
                bias_ipw_unadj = proportion_ipw - proportion_unadj,
                bias_orig_unadj = proportion_orig - proportion_unadj) %>% 
  dplyr::select(sdistri,population,indicator,contains("bias")) %>% 
  mutate(indicator = case_when(indicator == "S81" ~ "Stunting",
                               indicator %in% c("S84","S86","S87") ~ "Underweight",
                               indicator %in% c("S92","S97","S95") ~ "Anemia",
                               indicator %in% c("S89","S88") ~ "Overweight",
...
(nfhs4_ipw_estimates_district %>% 
    ...
    theme_bw() +
    theme(text = element_text(size = 8)))%>% 
  ggsave(.,filename = paste0(path_response_folder,"/figures/district jitter of bias_ipw_unadj.png"),width=2000,height=800,units="px")
